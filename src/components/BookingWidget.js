...
// ─── Feature 4: Interactive Component — BookingWidget ───────────────────────
//
// Displayed on the scooter detail page (/vehicles/[id]).
// Allows users to select rental dates, see a dynamic price breakdown in real-time,
// and create a reservation with zero deposit.
//
// Features:
//   - Live rental duration & price calculation: (days * price_per_day)
//   - Enforces valid future date ranges (pickup today/tomorrow, return after pickup)
//   - Auth-aware: prompts guests to "Log In to Book" and authenticated users to "Reserve Now"
//   - Displays confirmation receipt card upon successful reservation
//
// ────────────────────────────────────────────────────────────────────────────
...
// Helper: Format a Date object to YYYY-MM-DD string for <input type="date" />
...
export default function BookingWidget({ vehicle }) {
  ...
  // Initialize dates: pickup tomorrow, return 3 days later
  ...
  // ─── Dynamic Duration and Total Price Calculation ─────────────────────────
  ...
  // ─── View 1: Success Receipt Confirmation View ───────────────────────────
  if (bookingSuccess) {
    return (
      <div className="bg-white rounded-2xl p-6 border-2 border-green-200 shadow-sm space-y-4 text-center">
  ...
  // ─── View 2: Interactive Booking Form ────────────────────────────────────
  return (
    <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm space-y-5">
      ...
      {/* Error Alert */}
      {errorMsg && (
        <div className="bg-red-50 border border-red-200 text-red-700 text-xs p-3 rounded-xl flex items-start gap-1.5">
          <span>⚠️</span>
          <span>{errorMsg}</span>
        </div>
      )}
      ...
      <form onSubmit={handleBookingSubmit} className="space-y-4">
        ...
        {isValidRange ? (
          <div className="bg-sky-50/70 border border-sky-100 rounded-xl p-4 space-y-2 text-xs">
        ...
        {isAuthenticated ? (
          ...
          <div className="space-y-2">
            <Link
              href="/login"
              className="w-full inline-flex items-center justify-center bg-sky-500 hover:bg-sky-600 text-white font-semibold rounded-xl text-sm py-3 px-4 shadow-sm transition-colors"
            >
              Log In to Reserve
            </Link>
